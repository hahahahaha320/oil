...
public class BellmanFordSP {
    ...
    //持续V轮，每轮放松所有边E，则可以得到最短路径树
    //s -> w的最短路径，最多有V个顶点。没放松一轮，必然找到最短路径中的一个点，持续V轮，必然找到最短路径
    //改进：每一轮中的许多边的放松是无效的，只有上一轮的distTo[]值发生变换的顶点出发的边才能改变其他distTo[]的值

    //通俗的说，第一轮放松，我们会找到那些和顶点s只有一条边相连的顶点；第二轮放松，我们会找到那些和顶点s有两条边相连的顶点；
    //同理第n轮放松，我们会找到那些和顶点s有n条边相连的顶点，因为最多只有V-1条边，所有在V-1轮，就必然会找到所有的点；
    ...
    public static void main(String[] args) {
        String graphDataPath = "F:\\books\\algs4-data\\tinyEWDn.txt";
        In in = new In(graphDataPath);
